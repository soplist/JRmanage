...
public class ChangeequipmentBizImpl implements ChangeequipmentBiz {
	...
	public boolean update(Changeequipment ce) {
		String hql="update Changeequipment c set c.customer=:customer,c.carnumber=:carnumber,c.color=:color,c.phone=:phone,c.sim=:sim,c.terminal=:terminal,c.people.id=:peopleid,c.changedate=:changedate,c.cartype.id=:cartypeid,c.number=:number,c.registerdate=:registerdate,c.endpay=:endpay,c.remark=:remark where c.id=:id";
		Map<String, Object> params=new HashMap<String, Object>();
		params.put("id",ce.getId());
		params.put("customer",ce.getCustomer());
		params.put("carnumber",ce.getCarnumber());
