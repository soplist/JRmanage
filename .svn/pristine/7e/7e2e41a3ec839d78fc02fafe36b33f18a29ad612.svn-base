					...
					<div style="width: 100%;">
						<table style="width: 96%;margin-left:2%;border: solid 1px #c0cdd8;border-right: none;border-bottom: none;" cellpadding="1" cellspacing="0">
								...
								<tr>
									...
									<td class="td_right">
										<input id="money" name="money" readonly="true" type="text" style="overflow: hidden;width:87%;" >元
									</td>
								</tr>
								<tr>
									<td class="td_left">用途:
		...
		<div style="margin-left: 25px; height: 30px;float: left;">
			<table>
				<tr>
					...
					<td>
						<!-- 导出报表 -->
							<a href="pricarrecord_exportExcel.action" style="text-decoration: none;">
...
<script type="text/javascript">
	$(function() {
			...
			/* 为导出报表使用 */
			$.ajax({
				url: 'pricarrecord_list01.action',
				data:{},
				type:"post",
			    dataType:"json",   
			...
			/* 列表展示 */
			$("#dg").datagrid({
					'url' : 'pricarrecord_list.action',//请求的地址
			...
			$('#btn').bind('click', function(){
				...
				$.ajax({
						url: 'pricarrecord_findBytime.action',
			...
			/* $('#btn_sum').bind('click', function(){
					$.ajax({
						url: 'pricarrecord_sumByKm.action',
						type:"post",
					    dataType:"json",   
						success: function(data) {
  	...
  	function add(){
        ...
        initCombox03($('#pnameid'),'persionfile_findPeople.action',null,-1);
  		...
  		initCombox02($('#carid'),'privatecar_findcar.action',null,-1);
		//打开添加窗口
		$('#tj').dialog('open');
		...
		$(this).on('click',function(){
			...
			if(startkm>=1 && endkm>=startkm){
				...
				$("#km").val(km);
				$.ajax({
					url : 'privatecar_findByCarnumber.action',
					...
					success : function(data) {
						...
						if(data.output>0){
							$.ajax({
								url : 'global_findByOutput.action',
								data:{output:output},
								dataType : 'json',
								type : 'POST',
								success : function(data) {
									var money = km*(data.hkm*10)/10;
									$('#money').val(money);
								}
							});
						}
